...
from gpytorch.likelihoods import (
    BernoulliLikelihood,
    GaussianLikelihood as GaussianBase,
    MultitaskGaussianLikelihood as MultitaskGaussianBase,
)
...
class TunePrLikelihood(BaselineLikelihood):
    ...
    def expected_log_prob(self, observations, function_dist):
        ...
        if torch.any(observations.eq(-1)):
            raise RuntimeError("Labels should be in {0, 1}")
        # get samples
        ...
        def _log_prob_lambda(latent_samples):
            ...
            if self.training:
                ...
                log_debias_per_example = torch.index_select(
                    input=self.log_debias, dim=0, index=labels_bin * 2 + sens_attr
                )
...
def debiasing_params_target_calibration(flags):
    ...
    # P(y|y',s) shape: (y, s, y')
    label_posterior = np.array(
        [
            [[p_y0_ybar0_s0, p_y0_ybar1_s0], [p_y0_ybar0_s1, p_y0_ybar1_s1]],
            [[p_y1_ybar0_s0, p_y1_ybar1_s0], [p_y1_ybar0_s1, p_y1_ybar1_s1]],
        ]
    )
...
class MultitaskGaussianLikelihood(MultitaskGaussianBase):
    """Gaussian likelihood"""
